...
class JobSeekerApplicationController extends Controller
{
            ...
            ->addColumn('action', function($application){
                ...
                return
                    "<div class='d-flex align-items-center justify-content-around gap-2'>
                        ...
                        <form method='POST' action='" . route('job_seeker.application.withdrawn', $application) . "' style='display:inline-block'>
                            " . csrf_field() . "
                            " . method_field('PUT') . "
                            <button class='btn btn-danger btn-sm text-light' " . $isDisabled . ">
    ...
    public function withdrawn(Application $application)
    {
        $application->update([
